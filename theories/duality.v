(*
  ...
  Структурные условия сходимости фильтра Калмана (наблюдаемость,
  детектируемость, грамиан наблюдаемости) и парные им условия управления
  (управляемость, стабилизируемость, грамиан управляемости) - не два независимых
  набора, а образы друг друга при эрмитовом сопряжении $(dot)†$. Здесь это
  сделано явным: каждая лемма переводит объект оценивания в его двойственный по
  управлению заменой $(F, G) |-> (F†, G†)$. Для левого и правого критериев PBH
  собственное значение при этом сопрягается, $λ |-> λ^*$, а его модуль $|λ|$
  сохраняется (`norm_conjC`), поэтому граница неустойчивости $|λ| >= 1$
  переносится без изменений.

  Доказанные соответствия:
  - `stabilizable_dual` - стабилизируемость пары $(F, G)$ равносильна
    детектируемости двойственной пары $(F†, G†)$ (левый критерий PBH переходит в
    правый);
  ...
  Двойственность самих оптимальных решений уравнения Риккати (фильтр Калмана и
  LQR-регулятор) классическая и здесь не формализуется: для неё требуется
  отдельно определить управляющее уравнение Риккати (LQR), что вынесено в
...
Section Duality.
  ...
  *)
  ...
  (*
    ...
    Стабилизируемость с обратной связью по состоянию $F - G K$ (устойчивость по
    Шуру) равносильна стабилизируемости двойственной системы коррекцией по
    выходу $F† - K' H†$; эрмитово сопряжение сохраняет спектр по модулю.
