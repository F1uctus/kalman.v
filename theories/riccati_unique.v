(*
  Единственность PD-неподвижной точки апостериорного шага Риккати
  (книжная единственность стабилизирующего решения DARE,
  Kailath–Sayed–Hassibi гл. 14.5), БЕЗ внешних гипотез о
  стабилизирующем усилении.

  Ключ — книжная Lemma 14.5.3 для РАЗНОСТИ двух решений:

      M1 − M2 = Fp(M1) (M1 − M2) Fp(M2)†,

  где Fp(M) = F − F·Kf(M)·H — предсказательный замкнутый контур.
  Каждый Fp(Mi) Schur-устойчив (`lyap_inv_spec_rad`: PD предсказанная
  неподвижная точка с PD-весом `Kp Rn Kp† + G Q G†`), откуда
  `lyap_two_sided_zero_schur` даёт M1 − M2 = 0.

  Это устраняет `Hypothesis Pss_unique` в `dare.v`: разность любой
  PD-неподвижной точки и Pss зануляется.  Аксиомы — только `boolp`.
*)
...
From Kalman Require Import kalman spec_rad lyap_inv.
...
(* Усиление-тождество: Fp(M) M H† = Kp(M) Rn (ср. dare.v Fp_Ppss_Ht). *)
Lemma Fp_M_Ht (M : 'M[ℂ]_n) : psd M ->
...
(* F (I − Kf M H) = Fp M (свёртка усиления). *)
Lemma F_ImKfH (M : 'M[ℂ]_n) :
  F *m (1%:M - Kf M *m H) = F - F *m Kf M *m H.
Proof. by rewrite mulmxBr mulmx1 mulmxA. Qed.

(* Замкнуто-контурное тождество для предсказанной ковариации (книжная   *)
(* Lemma 14.5.3, для произвольной PSD предсказанной точки M).            *)
Lemma pred_closed_loop_id (M : 'M[ℂ]_n) : psd M ->
...
(* Разностное тождество для двух PSD предсказанных неподвижных точек     *)
(* (книжная Lemma 14.5.3 для разности двух решений):                     *)
(*   M1 − M2 = Fp(M1) (M1 − M2) Fp(M2)†.                                 *)
...
(* R1: A1 M1 A1† − A1 M1 A2† = P1 Rn P2† − P1 Rn P1†. *)
...
(* key1: A1 M1 A1† + P1 Rn P1† = A1 M1 A2† + P1 Rn P2†. *)
...
(* Замкнутый контур PD предсказанной неподвижной точки Schur-устойчив.    *)
Lemma Fp_schur (M : 'M[ℂ]_n) : pd M ->
...
have Mid : M = (F - F *m Kf M *m H) *m M *m (F - F *m Kf M *m H)^t*
              + (F *m Kf M *m Rn *m (F *m Kf M)^t* + G *m Q *m G^t*).
  by rewrite {1}Mfix (pred_closed_loop_id (pd_psd Mpd)) -addrA.
have Wpd : pd (F *m Kf M *m Rn *m (F *m Kf M)^t* + G *m Q *m G^t*).
  rewrite addrC; apply: pd_add; first exact: pd_GQGt.
  exact: psd_lcongr (F *m Kf M) (pd_psd Rn_pd).
exact: lyap_inv_spec_rad Mpd Wpd Mid.
Qed.

(* Единственность PD предсказанной неподвижной точки.                     *)
...
have D : M1 - M2
       = (F - F *m Kf M1 *m H) *m (M1 - M2) *m (F - F *m Kf M2 *m H)^t*
  := pred_diff_id (pd_psd q1) (pd_psd q2) f1 f2.
...
have M1pd : pd (predict_cov F G Q L1).
  rewrite /predict_cov addrC; apply: pd_add; first exact: pd_GQGt.
  exact: psd_lcongr F (pd_psd q1).
have M2pd : pd (predict_cov F G Q L2).
  rewrite /predict_cov addrC; apply: pd_add; first exact: pd_GQGt.
  exact: psd_lcongr F (pd_psd q2).
...
have HM : predict_cov F G Q L1 = predict_cov F G Q L2
  := pred_fix_unique M1pd M2pd M1fix M2fix.
