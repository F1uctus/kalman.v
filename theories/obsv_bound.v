(*
  ...
  - `obsv_gram k := ∑_(j<k) (F^j)† H† invR H F^j`
    (информационно-взвешенный грамиан наблюдаемости).
  ...
  ([kailath2000], § 14.5, через информационную форму P_k^(-1) >= obsv_gram k)
  здесь не доказана: она требует тождества Вудбери / явного выражения
  `invmx (predict_cov P)` через `invmx P` и `invmx Q`. Затравочная оценка при
  K=0 выше даёт ослабленную (зависящую от `F`) оценку, достаточную для
  построения предельного `Pss` при дополнительной гипотезе устойчивости.
