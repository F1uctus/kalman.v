(*
  ...
  Одношаговый шум: четырёхточечное симметричное распределение со значениями
  плюс минус α и плюс минус β на шестнадцати равновероятных исходах 'I_16
  (значениям α и минус α отвечают по пять исходов, значениям β и минус β по
  три). Среднее равно нулю, второй момент равен (5 α² + 3 β²) / 8; подбором
  α и β второй момент совпадает с заданной скалярной ковариацией шума.

  Совместная модель процесса шумов: равномерное распределение на функциях
  из конечного горизонта 'I_T в пространство исходов одного шага. Такое
  распределение равно произведению одношаговых равномерных распределений;
  маргинал каждой координаты равномерен. Ключевая лемма `sum_path_coord`
  (сумма значений функции одной координаты по всем траекториям) выводится
  из тождества `bigA_distr_bigA`.
...
Section FourPoint.
  ...
  (* Сумма квадратов значений: десять раз α² и шесть раз β². *)
...
Section PathSpace.
  ...
  (*
    ...
    По тождеству `bigA_distr_bigA` сумма значений функции от одной
    координаты по всем траекториям распадается в произведение: по
    координате j суммируются значения h, по каждой из остальных T - 1
    координат суммируется единица, что даёт множитель `#|A| ^ T.-1`.
  *)
  ...
  (*
    Маргинал координаты: ожидание функции одной координаты траектории
    равно ожиданию против одношагового равномерного распределения.
  *)
...
Section KalmanNoiseModel.
  ...
  (*
    ...
    координаты питают шум управления, нечётные шум измерения. Все
    вероятностные гипотезы теоремы `kalman.unbiased` дискретной моделью
    выполнены, что показывает следствие `model_unbiased`.
  *)
  ...
  (*
    ...
    тождественно; все остальные гипотезы теоремы `unbiased` (нулевое
    среднее шумов на каждом шаге) доказаны выше из равномерности
    распределения на траекториях исходов.
  *)
