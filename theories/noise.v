(*
  ...
  (значениям α и минус α отвечают по пять исходов, значениям β и минус β по три).
  Среднее равно нулю, второй момент равен (5 α² + 3 β²) / 8; подбором α и β
  второй момент совпадает с заданной скалярной ковариацией шума.
...
Section KalmanNoiseModel.
  ...
  *)
  ...
  (*
    ...
    тождественно; все остальные гипотезы теоремы `unbiased`
    (нулевое среднее шумов на каждом шаге) доказаны выше из равномерности
