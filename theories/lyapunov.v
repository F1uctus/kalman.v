(*
  ...
  1.  Матрица $Q$ должна быть эрмитовой и неотрицательно определённой (обычно
      задаёт внешние шумы или веса);
  2.  Дискретная система асимптотически устойчива тогда и только тогда, когда
      ...
      определённого $Q$ уравнение $X = A X A† + Q$ имеет единственное
      положительно определённое решение $X$;
  3.  При только неотрицательно определённой $Q$ решение $X$ также
      неотрицательно определено. Для положительной определённости нужна
      наблюдаемость пары $(A, Q^(1/2))$. Достаточное (более сильное) условие -
      $A$ есть сжатие в норме Фробениуса $(frob_sq A < 1)$, откуда сразу следует
      $|λ| < 1$.

  При таких условиях частичная сумма
  $"lyap_partial" A Q N := sum_(k < N) A^k Q (A†)^k$ монотонно возрастает в
  порядке Лёвнера и равномерно ограничена (в том же порядке) матрицей,
  пропорциональной единичной (эквивалентно, ограничена по спектральной норме).
  Поэтому через `mx_mono_lim` определён предел `lyap_sol A Q`, являющийся
  решением (неотрицательно определённой неподвижной точкой) уравнения Ляпунова.
  ...
  - Область неопределенности: В зависимости от ранга ковариационной матрицы это
    ...
    неограниченным вдоль некоторых направлений (цилиндр при наличии нулевых
...
Section LyapunovPartialSum.
  ...
  *)
  ...
  (*
    Тождество сдвига:
    $"lyap_partial" (N+1) = Q + A dot "lyap_partial" N dot A†$.
  *)
  ...
  (*
    ...
    Ляпунова $P = A P A† + Q$ её частичными суммами:
    $"lyap_partial" N prec.eq P$.
  *)
  ...
  *)
  ...
  (*
    ...
    поточечного обращения в ноль $Q ((A†)^j v) = 0$ из условия неотрицательной
  ...
  *)
  ...
  (*
    Аддитивность и разложение на блоки (для устойчивой по Шуру суммируемости).
  *)

  (*
    Аддитивность по индексу: частичная сумма на $[0, n+m)$ распадается на блок
    $[0, n)$ + сдвинутый блок $A^n · Σ_[0,m) · (A†)^n$.
  *)
  ...
  (* Монотонность частичной суммы по индексу. *)
...
(*
  ...
  матрицы $B := A^(N_0)$ с весом $W_0 := "lyap_partial" A Q N_0$. Это сводит
  случай устойчивости по Шуру к ограничению $"frob_sq" B < 1$.
*)
...
Section InputToStateStability.
  ...
  (*
    ...
    - @bernard2017["Formalization of the Lindemann-Weierstrass Theorem"].
  *)
