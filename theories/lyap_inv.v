(*
  ...
  единичном диске: любое собственное значение $lambda$ удовлетворяет $|lambda| <= 1$.

  При наблюдаемости пары $(A, H)$ с $Q = H† H$
  (соотв. управляемости $(A, G)$ с $Q = G G†$) спектр строго внутри:
  $|lambda| < 1$ - это и есть устойчивость по Шуру матрицы $A$.

  Доказательство - прямое спектральное
  (одна квадратичная форма на собственном векторе):
  $v† X v = |lambda|^2 v† X v + v† Q v => (1 - |lambda|^2) v† X v = v† Q v >= 0$, и
  $v† X v > 0$ ($X$ положительно определена, $v != 0$) даёт $|lambda|^2 <= 1$.
  Строгость следует из PBH-детектируемости: при $|lambda| = 1$ правая часть
...
Section LyapInv.
  ...
  (*
    ...
    Баланс квадратичной формы на собственном векторе $A v = lambda v$:
    $(1 - |lambda|^2)(v† X v) = v† Q v$.
  *)
  Lemma lyap_qf_balance n (X A Q : 'M[ℂ]_n) (lam : ℂ) (v : 'cV[ℂ]_n) :
    X = A^t* *m X *m A + Q ->
    A *m v = lam *: v ->
    (1 - `|lam| ^+ 2) * \tr (v^t* *m X *m v) = \tr (v^t* *m Q *m v).
  (* Схема: v† (A† X A) v = (A v)† X (A v) = |lam|² (v† X v). *)
  Proof.
    move=> lyapeq Fv.
  ...
  (* Без наблюдаемости: спектр в замкнутом единичном диске $|lambda| <= 1$. *)
  ...
  *)
  ...
  (*
    ...
    (левый собственный вектор $w A = lambda w$ <=> правый $A† w† = lambda^* w†$).
  ...
  *)
  ...
  (* Форма замкнутого контура, без управляемости: $|lambda| <= 1$. *)
  ...
  *)
  ...
  (*
    ...
    устойчивость по Шуру $|lambda| < 1$ следует напрямую: при $|lambda| = 1$ баланс
    зануляет $v† W v$, что невозможно для положительно определённого $W$ и
    $v != 0$. Это снимает нужду в наблюдаемости/детектируемости
    (ср. `lyap_eigval_lt1`).
  *)
  ...
  *)
  ...
  (*
    ...
    пары $(A, W)$ (левый PBH на $|lambda| >= 1$).
  ...
  *)
  Lemma lyap_inv_spec_rad_stab n (X A W : 'M[ℂ]_n.+1) :
    ...
    spec_rad_lt1 A.
  (*
    Энергетический баланс на левом собственном векторе $w A = lambda w$ даёт
    $(1 - |lambda|^2)(v† X v) = v† W v$; при $|lambda| >= 1$ левая часть <= 0, правая >=
    0, обе зануляются, откуда
  ...
  *)
  ...
  (*
    ...
    замкнутого контура с $|lambda| >= 1$ зануляет весь вес, то
    $w K_p R K_p† w† = 0 => w K_p = 0$ ($R$ положительно определена) и
    $w Z w† = 0 => w Z = 0$ ($Z$ неотрицательно определена), откуда
    $w (A - K_p H) = w A$, т.е. $w A = lambda w$ с $w Z = 0$ - противоречие со
