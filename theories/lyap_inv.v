(*
  ...
  При наблюдаемости пары $(A, H)$ с $Q = H† H$ (соотв. управляемости $(A, G)$ с
  $Q = G G†$) спектр строго внутри: $|λ| < 1$ - это и есть устойчивость по Шуру
  матрицы $A$.

  Доказательство - прямое спектральное (одна квадратичная форма на собственном
  векторе):
...
Section LyapInv.
  ...
  *)
  ...
  (*
    ...
    и $v != 0$. Это снимает нужду в наблюдаемости/детектируемости (ср.
    `lyap_eigval_lt1`).
  *)
  ...
  *)
  ...
  (*
    ...
    `spec_rad_lt1 A`. Каждый корень характеристического многочлена $A$ (левый
    собственный вектор) строго внутри единичного диска
  ...
  *)
  ...
  (*
    ...
    0, обе обращаются в ноль, откуда (через `psd_qf0_mul0` для неотрицательно
    определённых матриц) $w W = 0$ - противоречие со стабилизируемостью.
  *)
    Proof.
  ...
  (*
    ...
    переносится на замкнутый контур коррекции по выходу $A - K_p H$ с
    дополненным весом $K_p R K_p† + Z$ ($R$ положительно определена). Это
    @kailath2000[App. E, Lemma E.4.2 "Stable F − K_{P,Z} H"]: если левый
    собственный вектор $w$ замкнутого контура с $|λ| >= 1$ обращает в ноль весь
    вес, то $w K_p R K_p† w† = 0 => w K_p = 0$ ($R$ положительно определена) и
