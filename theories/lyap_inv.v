(*
  ...
  единичном диске: любое собственное значение `lam` удовлетворяет `|lam| <= 1`.

  При наблюдаемости пары `(A, H)` с `Q = H† H`
  (соотв. управляемости `(A, G)` с `Q = G G†`) спектр строго внутри:
  `|lam| < 1` - это и есть устойчивость по Шуру матрицы `A`.

  Доказательство - прямое спектральное
  (одна квадратичная форма на собственном векторе):
  `v† X v = |lam|² v† X v + v† Q v => (1 - |lam|²) v† X v = v† Q v >= 0`, и
  `v† X v > 0` (X положительно определена, `v != 0`) даёт `|lam|² <= 1`.
  Строгость следует из PBH-обнаруживаемости: при `|lam| = 1` правая часть
  зануляется, откуда `H v = 0`, что противоречит обнаруживаемости.
*)
...
Section LyapInv.
  ...
  *)
  ...
  (*
    С обнаруживаемостью пары (A, H), Q = H† H: строгая устойчивость по Шуру
    `|lam| < 1`.
  *)
  ...
  (*
    ...
    Двойственность PBH: управляемость (A, G) даёт обнаруживаемость (A†, G†)
  ...
  *)
  ...
  (* Форма замкнутого контура, без управляемости: `|lam| <= 1`. *)
  ...
  *)
  ...
  (*
    ...
    устойчивость по Шуру `|lam| < 1` следует напрямую: при `|lam| = 1` баланс
    зануляет `v† W v`, что невозможно для положительно определённого `W` и
    `v != 0`. Это снимает нужду в наблюдаемости/обнаруживаемости
    (ср. `lyap_eigval_lt1`).
  *)
  ...
  *)
  ...
  (*
    ...
    без положительной определённости
    ==================================================================
  *)
  ...
  *)
  ...
  (*
    ================================================================== Перенос
    стабилизируемости на замкнутый контур коррекции по выходу
    ==================================================================
  *)
  ...
  (*
    ...
    предсказательного контура `Fp = F − F Kf H`, из стабилизируемости пары
    `(F, G Q^½)`.
  *)
