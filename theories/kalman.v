...
Section KalmanFilter.
  ...
  *)
  ...
  (*
    ...
    как леммы, а на вещественном поле оператор совпадает с E из infotheo (лемма
    ExpE). Гипотезами остаются только содержательные вероятностные предположения
    о шумах: нулевое среднее на каждом шаге. Эти гипотезы выполнимы: в `noise.v`
    построена конкретная модель шумов (четырёхточечное распределение на
    равномерном пространстве траекторий исходов), для которой нулевые средние
    доказаны, а несмещённость для неё получается следствием `model_unbiased`.
  *)
