(*
  Обнаруживаемость, стабилизируемость, PBH-тест.
  ...
  состояния в любое другое за конечное время допустимым управлением; критерий
  Калмана - матрица управляемости полного ранга. Наблюдаемость: состояние
  полностью восстановимо по выходу; критерий Калмана - матрица наблюдаемости
  полного ранга.

  Здесь используется эквивалентный критерий Попова–Белевича–Хаутуса (PBH),
  проверяемый на собственных значениях матрицы `F`:

  - Критерий управляемости: пара (F, G) управляема <=> ∀ λ
    (собственного значения `F`) выполнено условие максимального ранга
    `rank [F − λE | G] = n`. Геометрический смысл: нет левого собственного
  ...
  - Критерий наблюдаемости: пара (F, H) наблюдаема <=> ∀ λ
    (собственного значения `F`) выполнено условие:
    ```
         ⎡F − λE⎤
    rank ⎣  H   ⎦ = n.
    ```
...
(* Обнаруживаемость (правый PBH-тест). *)
Section Detectability.
  ...
  (*
    `detectable F H` - правый PBH-тест: каждая неустойчивая мода наблюдаема.
  ...
  *)
  ...
  (*
    Наблюдаемость влечёт обнаруживаемость (нет неустойчивых ненаблюдаемых мод).
  ...
  *)
  ...
  (*
    Определение (обнаруживаемость): существует коррекция по выходу K, делающая
    замкнутый контур `F − K H` устойчивым по Шуру.

    ([kailath2000], App. C, § C.4; [wonham1985], § 2.2, Thm 2.2)
  *)
  Definition detectable_stabilizing : Prop :=
    exists K : 'M[ℂ]_(n, p), spec_rad_lt1 (F - K *m H).

  (*
    Тривиальное направление: стабилизируемость замыкания => PBH. Если `F − K H`
    устойчива по Шуру и `F v = λ v` с `|λ| >= 1`, то при `H v = 0` имеем
    `(F − K H) v = λ v` - собственное значение вне единичного круга, что
    противоречит устойчивости по Шуру (`spec_rad_lt1_eigval`).
  *)
...
Section Stabilizability.
  ...
  (*
    `stabilizable F G` - левый PBH-тест: каждая неустойчивая мода управляема.
  ...
  *)
  ...
  (*
    Тривиальное направление (двойственно обнаруживаемости, левый PBH): если
...
Section SchurStableTrivialDet.
  ...
  (*
    ...
    пара `(F, H)` обнаруживаема, потому что у F нет неустойчивых мод.
...
(*
  ...
  (PBH-обнаруживаемость влечёт существование стабилизирующего K) - теорема о
  назначении полюсов (pole placement) для обнаруживаемой пары. Полное
  доказательство (через наблюдаемую форму / форму Бруновского) в mathcomp/CoqQ
  отсутствует; [kailath2000] цитирует pole placement из стандартной литературы,
  но не доказывает его
  (это двойственная к стабилизируемости коррекция по выходу, App. C, § C.4).
...
(* Обнаруживаемость переносится на пару `(F, H F)`. *)
...
(*
  Обнаруживаемость пары `(F, H)` даёт стабилизирующее усиление `K0`, делающее
  апостериорный замкнутый контур `Mc = (E − K0 H) F` устойчивым по Шуру.
...
(*
  Схема без коспектральности и обратимости `F`: `(E − K0 H) F = F − K0 (H F)`,
  поэтому достаточно стабилизировать предсказательный контур пары `(F, H F)`
  (которая обнаруживаема по `detectable_mulHF`), что и даёт
