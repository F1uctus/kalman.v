...
(*
  Устойчивость по Шуру.

  Для дискретной системы она совпадает с асимптотической устойчивостью:
  спектральный радиус строго меньше единицы, то есть все собственные значения
...
Section Stabilizability.
  ...
  (*
    ...
    Каждый левый собственный вектор $w$ матрицы $F$ с $|λ| >= 1$ возмущается
    входом: $w G != 0$.
...
(*
  ...
  нет левого собственного вектора $w != 0$ с $w M = 0$ (иначе $w M w† = 0$
  противоречит положительной определённости $M$).
*)
...
(*
  Двойственное направление для стабилизируемости (назначение полюсов через
  обратную связь по состоянию).
...
(*
  ...
  $"detectable_mulHF"$; стабилизирующее усиление получается из
