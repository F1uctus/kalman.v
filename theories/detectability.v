...
Section Detectability.
  ...
  *)
  ...
  (*
    Наблюдаемость => детектируемость (нет неустойчивых ненаблюдаемых мод).
  ...
  *)
  ...
  (*
    Определение (Детектируемость). - существует коррекция по выходу K, делающая
...
(*
  ...
  (PBH-детектируемость => существование стабилизующего K) - это теорема о
  назначении полюсов (pole placement) для детектируемой пары. Полное
  ...
  (это двойственная к стабилизируемости коррекция по выходу - state-feedback test, App. C, § C.4).
...
(*
  ...
  (назначение полюсов через обратную связь по состоянию, state-feedback test).
...
(*
  Детектируемость пары `(F, H)` даёт стабилизирующее усиление `K0`, делающее
...
(*
  ...
  (которая детектируема по `detectable_mulHF`), что и даёт
