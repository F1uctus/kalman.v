(* Топологические свойства матриц + поэлементная сходимость.                  *)
(*                                                                            *)
(* - определяется сходимость последовательности матриц через                  *)
(*   сходимость по каждому элементу;                                          *)
(* - доказывается эквивалентность с матричной топологией из                   *)
(*   mathcomp-analysis (matrix_normedZmodType);                               *)
(* - устанавливается непрерывность сложения, умножения, эрмитова сопряжения   *)
(*   и следа.                                                                 *)
(*                                                                            *)
(* Норма Фробениуса используется как мост между скалярной и матричной         *)
(* топологией: frob_sq (M_k - L) → 0 => M_k → L. Это нужно для монотонной   *)
(* Сходимости последовательностей неотрицательно определённых матриц.         *)
...
(* Поэлементная сходимость и её эквивалентность с матричной топологией. *)
Section Entrywise.
...
Section FrobToMxCvg.
  ...
  (* Технический шаг: для ℂ-значной последовательности s ≥ 0, если
    s ≤ t и t → 0 в ℂ^o, то s → 0 в ℂ^o. *)
...
(* Вспомогательный факт: изометрия непрерывна.
   ...
   `ℂ^o` (это известная боль при работе с `regular_topology`). *)
...
(* Непрерывность матричных операций.
   ...
   `pseudoMetricNormedZmodType`-инстанс для `'M[ℂ]_(r, c)`.

   Для произведения, следа и эрмитова сопряжения сводимся
   к поэлементной сходимости (через `mxcvgnP`). На скалярном уровне
   арифметика над `ℂ : numClosedFieldType` требует обёртки `^o`,
...
Section Continuity.
  ...
  (* Сходимость суммы скалярных последовательностей.
     ...
     ℂ^o), поэтому идём через явное приведение к ℂ^o. *)
  ...
  (* Эрмитово сопряжение непрерывно: каждый элемент
...
(* ================================================================== *)
(*  Архимедова сходимость скалярных степеней (Session 20.5)            *)
(* ================================================================== *)
(*  Геометрическое затухание r^+k → 0 при 0 ≤ r < 1.                   *)
(*  Над numClosedFieldType (поле не упорядочено тотально) стандартная   *)
(*  cvg_expr из sequences.v неприменима — она требует archiRealFieldType,*)
(*  а замкнутое поле им быть не может.  Поэтому архимедовость берётся    *)
(*  как явная гипотеза, а затухание доказывается напрямую через         *)
(*  бином Бернулли + натуральный мажорант (truncn-свидетель).           *)
Section MxCvgArchi.
...
(*   k.+1 (1 - r) r^k ≤ (r + (1 - r))^(k+1) = 1.                        *)
(* Извлекаем i = 1 член бинома Ньютона (bigD1), остальные ≥ 0.          *)
...
(* Геометрическое затухание степеней: r^+k.+1 → 0 при 0 ≤ r < 1.        *)
(* Через ε–N: r^+k.+1 ≤ r^+k ≤ (k.+1 (1 - r))^-1 (Бернулли),           *)
(* и (k.+1 (1 - r))^-1 < ε при k.+1 > (ε (1 - r))^-1 (archi_natbound).  *)
