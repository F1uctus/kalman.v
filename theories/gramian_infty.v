...
From Kalman Require Import mxherm mxtopo mxmonotone lyapunov.
...
Section GramianInf.
  ...
  *)
  ...
  (*
    Связь конечного `obsv_gram F H E` (без R-веса) с `lyap_partial`. Используем
    существующий `obsv_gram` с единичной матрицей `E = R`.
  *)
...
Section ClosedLoopCtrlGramPd.
  ...
  *)
  ...
  (*
    ...
    частичной суммы зануляется,
    откуда `W (Fp†)^j v = 0` ∀j<N. Так как `W u = 0 => Kp† u = 0`
    (Rn положительно определена) и `Z u = 0` (Z неотрицательно определена), а
    `Kp† u = 0 => Fp† u = A† u`, по индукции `(Fp†)^j v = (A†)^j v`, значит
    `Z (A†)^j v = 0`; транспонирование даёт ядро управляемости `(A, Z)`, откуда
    `v = 0` - т.е. `P` положительно определена.
  *)
  Proof.
    ...
    *)
    ...
    (*
      Перенос на исходную пару (A, Z): транспонирование даёт ядро управляемости.
    *)
    have vT0 : v^t* = 0.
      apply: Hctrl => i.
