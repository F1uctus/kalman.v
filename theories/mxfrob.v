...
Section Frob.
  ...
  (*
    Если для любого $eps > 0$ имеем $frob_sq < eps$, то $frob_sq = 0$, откуда
    будет $M = 0$. Это нужно для отделимости метрики Фробениуса: разные матрицы
    лежат на положительном расстоянии.
  *)
