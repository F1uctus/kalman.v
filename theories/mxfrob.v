...
Section Frob.
  ...
  *)
  ...
  (*
    Мажоранта следа сопряжения неотрицательно определённой матрицы: tr
    (F M Fconj) <= frob_sq F * tr M при psd M.

    Доказательство: спектральное разложение M = U diag(l) Uconj, l_i >= 0; для W
    := (F U)conj (F U) (неотрицательно определена) получаем tr (F M Fconj) = ∑_i
    l_i W_ii <= ∑_i l_i (tr W) = (tr W)(∑_i l_i) = frob_sq F * tr M, где W_ii <=
    tr W (W неотрицательно определена => диагональ <= след) и tr W = frob_sq F.
    Это заменяет общую субмультипликативность Фробениуса: нужный частный случай
    выводится напрямую из спектральной теоремы.
  *)
