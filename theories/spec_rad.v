...
Section SpecRad.
  ...
  *)
  ...
  (*
    ...
    корней $A$ ($lambda E - A† = (lambda^* E - A)†$, а det эрмитовой = сопряжению det), а
...
(*
  ...
  A^k -> 0 при frob_sq A < 1. Объединяет алгебраические оценки frob_sq_exp_le с
  архимедовым затуханием r^k -> 0 (mxtopo.r_pow_cvgn0) через теорему о двух
  милиционерах и переход к норме Фробениуса (`frob_sq_cvgn0_to_mxcvgn`).
  Архимедовость - явная гипотеза
...
(*
  ...
  Схема @kailath2000[App. E, шаг 2]: через разложение Шура для A = U† T U сводим
  к T (треугольной). Для верхнетреугольной формы (транспонируем `is_trig_mx`)
  ведём индукцию по размеру, выделяя первый индекс блоком 1+n. Диагональ T^k и
  нижний правый блок сходятся к нулю по предположению индукции, а
  наддиагональный блок `ursubmx (T^k)` подчиняется скалярной ISS-рекурсии
  $S_(k+1) = a S_k + r (T')^k$ (поэлементно), которую замыкает `lin_filter_cvgn0`
  (lyapunov.v). Архимедовость нужна только для геометрического затухания
  $r^k -> 0$.
  ...
  *)
  ...
  Section SchurLyapunovSolution.
    ...
    (*
      ...
      оптимальности: $P^circle_i <= Pi$ через конечный Грамиан замкнутого контура.
