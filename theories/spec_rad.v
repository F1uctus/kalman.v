...
Section DiagBound.
  ...
  (*
    Для каждого диагонального элемента $M_(i i)$:
    $norm(M_(i i))^2 <= "frob_sq" M$. Тривиально через frob_sqE:
    $sum_(i,j) M_(i j)^* M_(i j) >=$ единичное слагаемое
    $M_(i i)^* M_(i i) = norm(M_(i i))^2$.
  *)
...
Section SpecRad.
  ...
  (*
    Спектральный радиус строго меньше единицы.

    Определение записано как существование разложения Шура с диагональю строго
    внутри открытого единичного диска. Оно не строит максимум модулей
    собственных значений, но его достаточно для последующих результатов:
    спектральный анализ ведётся через конкретную треугольную форму.
  ...
  *)
  ...
  (* Сжатие в норме Фробениуса $=>$ Устойчивость по Шуру. *)
  Theorem frob_sq_contract_spec_rad_lt1 n (A : 'M[ℂ]_n.+1) :
    frob_sq A < 1 -> spec_rad_lt1 A.
  (*
    Для $n + 1$: устраняем граничный случай $n = 0$ (устойчивость по Шуру
    требует $n > 0$).
  *)
  Proof.
    move=> Ac.
    ...
    *)
    ...
    (*
      $‖T_(i i)‖^2 < 1 => ‖T_(i i)‖ < 1$ - через mono `ltr_pXn2r` на Num.nneg.
    *)
  ...
  (*
    ...
    пара $(F, H)$ с замыкающим $K$, делающим $F − K H$ устойчивой по Шуру, не
    имеет ненаблюдаемых неустойчивых мод.
  ...
  *)
  ...
  (*
    ...
    треугольна, её $det = ∏_i (λ − T_(i i))$. Если $det = 0$, то $λ = T_(i i)$
    для некоторого i, откуда $|λ| = |T_(i i)| < 1$.
  *)
  Proof.
    ...
    (* $λ E − T$ треугольна $=> det = ∏_i (λ − T_(i i))$. *)
  ...
  *)
  ...
  (*
    ...
    корней $A$ ($λ E - A† = (λ^* E - A)†$, а $det$ эрмитовой = сопряжению
    $det$), а модуль при сопряжении не меняется.
  *)
  Proof.
...
Section SchurPow.
  ...
  (*
    ...
    $"frob_sq" (A B) <= "frob_sq" A dot "frob_sq" B$.
  *)
  Lemma frob_sq_mulmx_le n m p
      (A : 'M[ℂ]_(n, m)) (B : 'M[ℂ]_(m, p)) :
    frob_sq (A *m B) <= frob_sq A * frob_sq B.
  (*
    Схема: $"tr"((A B)† A B) = "tr"(B† (A† A) B)$; `tr_conj_frob_le` с
    $"Fm" := B†$, $M := A† A$ (неотрицательно определённая по psd_frob) даёт
    $"tr"(B† (A† A) (B†)†) <= "frob_sq" B† dot "tr"(A† A)$. $(B†)† = B$ (trmxCK)
    и $"frob_sq" B† = "frob_sq" B$ (frob_sq_trmxC), $"tr"(A† A) = "frob_sq" A$
    (по определению).
  *)
  Proof.
  ...
  (*
    Оценка нормы Фробениуса степени: $"frob_sq" A^(k+1) <= ("frob_sq" A)^(k+1)$.
  *)
  ...
  *)
  Lemma schur_exp_conj n (U A T : 'M[ℂ]_n) (k : nat) :
    ...
    A ^+ k = U^t* *m T ^+ k *m U.
  (*
    Доказательство: индукция по $k$; шаг использует $U dot U† = E$ для
    сокращения среднего множителя $U dot U†$ в произведении.
  *)
  Proof.
    move=> hU hA.
...
(*
  Сходимость прогрессии при сжатии по норме Фробениуса.
...
Section SchurPowCvg.
  ...
  *)
  ...
  (*
    Связка двух нитей пути @kailath2000[App. E]: сжатие по норме Фробениуса даёт
    одновременно устойчивость по Шуру (spec_rad_lt1) и сходимость степеней к
    нулю.
  *)
...
(*
  Сходимость степеней устойчивой по Шуру матрицы к нулю:
  ...
  Диагональ $T^k$ и нижний правый блок сходятся к нулю по предположению
  индукции, а наддиагональный блок `ursubmx (T^k)` подчиняется скалярной
  ISS-рекурсии $S_(k+1) = a S_k + r (T')^k$ (поэлементно), которую замыкает
...
Section SchurStablePowCvg.
  ...
  (* База индукции: $1 times 1$ матрица. *)
  ...
  (*
    Индукция по размеру через разложение по первой строке и столбцу.
    Используются условия верхнетреугольности $T†$, и то, что вся диагональ
    находится строго в единичном диске.
  *)
  Proof.
  ...
  (*
    Степени устойчивой по Шуру матрицы сходятся к нулю.
  ...
  *)
  ...
  (*
    Через разложение Шура $A = U† T U$ с нижнетреугольной матрицей $T$:
    транспонирование сводит к верхнетреугольному случаю. Далее индукция по
    размеру через разложение по первой строке и столбцу. Используется условие
    верхнетреугольности $T†$, и то, что вся диагональ находится строго в
    единичном диске.
  *)
  Proof.
  ...
  (*
    ...
    Однородное тождество замкнутого контура, итерированное по степеням:
    $D = A D B†$ влечёт $D = A^k D (B^k)†$ для всех $k$.
  *)
  ...
  *)
  ...
  Section SchurLyapunovSolution.
    ...
    (*
      ...
      оптимальности: $P^degree_i <= Pi$ через конечный Грамиан замкнутого
      контура.
