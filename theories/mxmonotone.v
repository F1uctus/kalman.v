(*
  ...
  (`S k <= S k.+1`) и ограничена сверху матрицей `B` (`S k <= B`), то `S`
  ...
  восстанавливаются через эрмитовы формы с тестовыми векторами `δ_i + δ_j` и
  `δ_i + 'i δ_j`.
*)
...
Section MxSeqMonotoneCvg.
  ...
  *)
  ...
  (*
    ...
    Доказательство: для фиксированного `k0` сдвиг `j |-> S (k0 + j)` сходится к
    той же матрице `mx_mono_lim`. Так как последовательность монотонна, все её
...
(*
  ...
  Двойственно к `MxSeqMonotoneCvg`: для убывающей посл-ти `S k+1 <= S k`
  доказываем сходимость через подстановку `T k := S 0 - S k`
...
Section MxSeqMonotoneCvgDec.
  ...
  *)
  ...
  Lemma mx_mono_dec_lim_psd :
    psd mx_mono_dec_lim.
  (*
    `mx_mono_dec_lim = S 0 - mx_mono_lim T`. Поскольку `lim T <= S 0`
    (через `mx_mono_lim_le`), `psd (S 0 - lim T)`.
  *)
  Proof.
    rewrite /mx_mono_dec_lim.
  ...
  Proof.
    ...
    (*
      Перепишем: `lim_dec = S 0 - lim T, S k0 = S 0 - T k0`. Тогда
      `S k0 - lim_dec = lim T - T k0 >= 0` (`HT_term`).
    *)
