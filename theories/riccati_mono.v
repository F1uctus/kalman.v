(*  Монотонность шагов фильтра Калмана в PSD-порядке Лёвнера.            *)
(*                                                                       *)
(*  В этом файле:                                                        *)
(*    * `predict_cov_mono`  — безусловная монотонность шага              *)
(*      предсказания: `predict_cov` линеен в `P`, член `G *m Q *m G^t*`  *)
(*      сокращается в разности, остаётся `F *m (P2 - P1) *m F^t*`.       *)
(*    * `update_cov_mono` — монотонность шага обновления на всём          *)
(*      PSD-конусе.  Доказывается через тождество оптимальности           *)
(*      `alt_update_cov_diff`:                                            *)
(*        update_cov(P1) ≤ alt_update_cov(K2, P1)                       *)
(*                       ≤ alt_update_cov(K2, P2) = update_cov(P2),     *)
(*      где K2 := kalman_gain(P2).  Линейность `alt_update_cov` в `P`    *)
(*      даёт среднее неравенство.                                        *)
(*    * `riccati_step_mono` — комбинация двух монотонностей; монотонна  *)
(*      на PSD-конусе.                                                  *)
(*    * `riccati_iter_mono_from_0` — последовательность                  *)
(*      `iter k riccati_step 0` неубывающая в PSD-порядке.  База         *)
(*      `iter 0 = 0 ≤ iter 1 = riccati_step 0` — это psd-ность           *)
(*      `riccati_step 0`; шаг — `riccati_step_mono`.  Никаких            *)
(*      дополнительных гипотез наблюдаемости/управляемости здесь не       *)
(*      нужно, поскольку `update_cov_mono` действует на PSD, а не на PD.  *)
(*                                                                       *)
(*  Зависимости: импорт `kalman.v` (за определениями и алгебраическими   *)
(*  тождествами шагов) и `spectral.v` (за `pd_inv_antimono`).            *)
...
From Kalman Require Import psd_base psd_order spectral kalman.
...
Lemma predict_cov_mono (P1 P2 : 'M[C]_n) :
...
rewrite eq.
exact: psd_mulmx_row hLe.
Qed.

(* ================================================================== *)
(*  Монотонность шага обновления на всём PSD-конусе                     *)
(*  (через тождество оптимальности усиления Калмана).                   *)
(*                                                                      *)
(*  Идея: для любого альтернативного усиления `K'` имеем                *)
(*    alt_update_cov K' P2 - alt_update_cov K' P1                        *)
(*       = (I - K'H) *m (P2 - P1) *m (I - K'H)^t*  ∈ PSD,                *)
(*  то есть `alt_update_cov K' (·)` монотонен в `P`.                     *)
(*  Положив `K2 := kalman_gain P2`, получаем                            *)
(*    alt_update_cov K2 P2 = update_cov P2                              *)
(*  (член `(K2 - K2) *m S2 *m (K2 - K2)^t*` исчезает),                  *)
(*  и                                                                    *)
(*    update_cov P1 ≤ alt_update_cov K2 P1                              *)
(*  (член `(K2 - K1) *m S1 *m (K2 - K1)^t*` ∈ PSD).                    *)
(*  Транзитивностью получаем `update_cov P1 ≤ update_cov P2`.          *)
(* ================================================================== *)

Lemma update_cov_mono (P1 P2 : 'M[C]_n) :
...
(* Шаг 3: alt_update_cov K2 P1 ≤ alt_update_cov K2 P2 (линейно в P).
   Доказываем через `psd_le_congr` (конгруэнтность сохраняет PSD-порядок)
...
(* ================================================================== *)
(*  Монотонность одного шага Риккати на PSD-конусе.                     *)
(* ================================================================== *)

Lemma riccati_step_mono (P1 P2 : 'M[C]_n) :
  psd P1 -> psd P2 -> psd_le P1 P2 ->
...
(* ================================================================== *)
(*  Бутстрэп от 0: последовательность `iter k riccati_step 0`           *)
(*  неубывающая в PSD-порядке.                                         *)
(*                                                                      *)
(*  База `psd_le 0 (riccati_step 0)` — это просто psd-ность             *)
(*  `riccati_step 0` (получается через форму Джозефа).                  *)
(*  Шаг — `riccati_step_mono`, применённый к IH.                        *)
(*                                                                      *)
(*  Замечательно, что **никакая гипотеза вида `pd_after_step1` не       *)
(*  требуется**: `update_cov_mono` действует на PSD-конусе, а не на PD.  *)
(* ================================================================== *)

Lemma riccati_step_psd (P : 'M[C]_n) :
...
Proof.
have psd0n : psd (0 : 'M[C]_n) := psd0 C n.
elim: k => [|k IH].
...
(* Следствие: PSD-ность всех итераций (уже было в kalman.v как
   `riccati_iter_psd`; перепакуем под нашу нотацию `riccati_step`). *)
Lemma riccati_iter_from_0_psd (k : nat) :
  psd (iter k (riccati_step F G H Q R) 0).
Proof. exact: (riccati_iter_psd F G H Q_psd R_pd k (psd0 C n)). Qed.
