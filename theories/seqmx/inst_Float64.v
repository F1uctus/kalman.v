(*
  ...
  применяется, и никакого доказательства совпадения с оператором `riccati_step`
  здесь нет. Каждая операция округляет результат к ближайшему представимому
  числу, так что даже кольцевые тождества выполняются лишь приближённо.
  Доказанная линия уточнения проходит по `rat` и `bigQ`: `inst_rat.v` связывает
  программу со спецификацией напрямую, `inst_bigQ.v` через гетерогенное
  отношение `RseqmxC r_ratBigQ`, а `inst_Q.v` сверяет с ними значения над Q.
  ...
  Отрицание задано как $0 - x$, а не примитивной операцией `PrimFloat.opp`:
  на нулевом аргументе `opp` даёт минус ноль, который попадал бы в числовой
  вывод отдельным представлением нуля.
*)
...
(*
  ...
  Наблюдаемые отклонения не превосходят 1.6e-14, то есть допуск выше их
