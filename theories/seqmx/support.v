(*
  ...
  библиотеки CoqEAL, скалярная матрица по `iota`, вложение натуральных чисел в
  коэффициентный тип, сопряжённое транспонирование и степень матрицы, а также
  базовые теоремы `refines`, связывающие эти программы с матрицами MathComp.

  Скалярная матрица по iota и eqn.

  CoqEAL строит диагональные матрицы через mkseqmx_ord с перечислением ординалов
  'I_n; перечисление проходит через непрозрачные доказательства вида reflect
  (idP и подобные), которые CertiRocq оставляет аксиомами: reflect лежит в Set,
  но тела непрозрачных доказательств недоступны стиранию MetaRocq. Построение по
  iota вычислительно чище: замыкание не содержит структур eqType, и один и тот
  же терм лучше поддаётся извлечению в целевые языки без промежуточных слоёв.
  Совпадение с построением CoqEAL даёт iscalar_seqmxE.
*)
