(*
  ...
  Все данные прогона строятся внутри Rocq: системные матрицы, начальные
  условия, шумы, истинная траектория, измерения, оценки и ковариации.
  Шумы порождаются четырёхточечной моделью из `noise.v` (нулевое среднее,
  второй момент равен заданной скалярной ковариации); конкретная траектория
  исходов задаётся детерминированным генератором Лемера над двоичными
  натуральными числами (Stdlib `N`), то есть прогон есть значение случайного
  процесса в одной точке вероятностного пространства из `noise.v`.
  Матричная арифметика шага выполняется извлекаемыми seqmx-программами из
  `riccati_seqmx.v`; OCaml-драйвер лишь переводит результат извлечённого
  терма в JSON.

  Чётные элементы потока исходов питают шум управления, нечётные шум
  измерения; это соответствует пространству траекторий {ffun 'I_(2T) -> 'I_16}
  из `noise.v`.
*)
...
(*
  ...
  поэтому младшие биты пригодны; исход одного шага есть s' mod 16.
  Вычисления ведутся в двоичных натуральных числах `N`, что извлекается в
  эффективный код.
*)
...
Definition sim_P0 : @seqmx C := [:: [:: cnat 2; 0%C]; [:: 0%C; cnat 2]].
Definition sim_x0 : @seqmx C := [:: [:: 1%C]; [:: 1%C]].

(*
  Значения четырёхточечного шума (`noise.v`, noise_val): α на исходах
  меньше пяти, минус α до десяти, β до тринадцати, иначе минус β.
*)
...
(*
  Один шаг прогона по исходам (ow, ov): эволюция истинного состояния,
  измерение, полушаги предсказания и обновления оценки, шаг Риккати для
  ковариации. Дословно соответствует `x_true`, `x_hat` и `kf_step` из
  `kalman.v` при u = 0.
*)
...
(*
  Полный прогон на T шагов: строка начального состояния (измерения ещё нет,
  вместо него пустая матрица), затем T шагов фильтра.
*)
Definition kalman_sim_run (T : nat) : seq sim_row :=
  let st0 := (sim_x0, seqmx0 2 1, sim_P0) in
...
(*
  ...
  Точные вычисления длинного прогона ведутся над `bigQ` (двоичные целые),
  как и в остальной части проекта: `bigQ` есть вычислимый внутри Rocq аналог
...
Section BigQSim.
  ...
  (*
    Проверка коридора для строки прогона: квадрат ошибки по компоненте
    положения не превосходит четырёх дисперсий, то есть ошибка лежит в
    коридоре плюс минус два сигма.
  *)
  ...
  (*
    Формальная проверка иллюстрации: на всём прогоне в сорок шагов ошибка
    оценки положения остаётся в коридоре плюс минус два сигма, построенном
