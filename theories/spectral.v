...
(*                                                                       *)
(*  Используется напрямую mathcomp.algebra.spectral.orthomx_spectralP:   *)
...
From Kalman Require Import psd_base psd_order.
...
Lemma psd_hermsym n (M : 'M[C]_n) : psd M -> M \is hermsymmx.
Proof.
case=> Msym _; apply/is_hermitianmxP.
by rewrite expr0 scale1r -Msym.
Qed.

Lemma pd_hermsym n (M : 'M[C]_n) : pd M -> M \is hermsymmx.
Proof. by move/pd_psd/psd_hermsym. Qed.

Lemma hermsym_eq n (M : 'M[C]_n) : M \is hermsymmx -> M = M^t*.
Proof.
move/is_hermitianmxP; rewrite expr0 scale1r => <-.
by [].
Qed.

Lemma unitary_mulV n (U : 'M[C]_n) : U \is unitarymx -> U^t* *m U = 1%:M.
Proof.
move=> hU.
...
Lemma hermsym_congr n (U M : 'M[C]_n) :
...
Definition diag_of n (l : 'I_n -> C) : 'M[C]_n :=
  \matrix_(i, j) (if i == j then l i else 0).

Lemma diag_of_hermsym n (l : 'I_n -> C) :
...
Lemma diag_of_const1 n : diag_of (fun _ : 'I_n => (1 : C)) = 1%:M.
Proof.
apply/matrixP=> i j; rewrite !mxE.
by case: (i == j); rewrite ?mulr1n ?mulr0n.
Qed.

Lemma diag_of_mul n (f g : 'I_n -> C) :
...
(* ================================================================== *)
(* Спектральная теорема как обёртка над mathcomp                      *)
(* ================================================================== *)

Lemma spectral_theorem n (M : 'M[C]_n) :
  M \is hermsymmx ->
  exists (U : 'M[C]_n) (l : 'I_n -> C),
...
pose l : 'I_n -> C := fun i => spectral_diag M ord0 i.
...
(* ================================================================== *)
(* Квадратичные формы и характеризация PSD через спектр               *)
(* ================================================================== *)

Lemma diag_of_qform n (l : 'I_n -> C) (v : 'cV[C]_n) :
...
Lemma psd_diag_iff_real n (l : 'I_n -> C) :
...
- case=> _ hQ i.
  ...
  have del1 : (delta_mx i ord0 : 'cV[C]_n) i ord0 = 1
    by rewrite mxE !eqxx /=.
...
Lemma psd_iff_eigvals_nn n (M : 'M[C]_n) :
  M \is hermsymmx ->
  (psd M <->
   exists (U : 'M[C]_n) (l : 'I_n -> C),
     U \is unitarymx /\ (forall i, l i \is Num.real) /\
...
(* ================================================================== *)
(* Сопряжение X ↦ U X U^t*                                           *)
(* ================================================================== *)

Lemma spec_conj_mul n (U D1 D2 : 'M[C]_n) :
  U \is unitarymx ->
...
Lemma spec_conj_one n (U : 'M[C]_n) :
  U \is unitarymx -> U *m 1%:M *m U^t* = 1%:M.
Proof. by move=> hU; rewrite mulmx1; have /unitarymxP -> := hU. Qed.

Lemma invmx_spec_conj n (U : 'M[C]_n) (l : 'I_n -> C) :
...
Lemma UAU_decomp n (A U : 'M[C]_n) (l : 'I_n -> C) :
...
Lemma pd_eigvals_pos n (A U : 'M[C]_n) (l : 'I_n -> C) :
...
case: pdA => _ pdAq.
pose ei : 'cV[C]_n := delta_mx i (ord0 : 'I_1).
pose v : 'cV[C]_n := U *m ei.
have eiNZ : ei != 0.
...
have lqf : \tr (ei^t* *m diag_of l *m ei) = l i.
  rewrite diag_of_qform (bigD1 i) //= big1 ?addr0; last first.
    move=> j /negbTE neji.
    by rewrite !mxE neji /= conjC0 mul0r mulr0.
  have del1 : (ei : 'cV[C]_n) i ord0 = 1 by rewrite mxE !eqxx /=.
  by rewrite del1 conjC1 mul1r mulr1.
suff: 0 < \tr (v^t* *m A *m v) by rewrite hVAV lqf.
exact: pdAq.
Qed.

Lemma psd_spec_conj_inj n (V X : 'M[C]_n) :
...
have he : V^t* *m (V *m X *m V^t*) *m V = X.
  ...
  by rewrite e2 !h !mul1mx -mulmxA h mulmx1.
by rewrite -he; apply: psd_congruence.
Qed.

(* ================================================================== *)
(* Главная теорема: антимонотонность обращения на PD                  *)
(* ================================================================== *)

Theorem pd_inv_antimono n (A B : 'M[C]_n) :
...
pose isl : 'I_n -> C := fun i => (sqrtC (l i))^-1.
pose S : 'M[C]_n := U *m diag_of isl *m U^t*.
...
have hIB' : psd_le 1%:M B'.
  ...
  rewrite {1}Ssym; exact: psd_congruence.
...
have diff_eq : invmx A - X =
  ...
  have factor :
    (S *m V) *m ((1%:M : 'M[C]_n) - diag_of (fun i : 'I_n => (lam i)^-1))
      *m (S *m V)^t* =
    (S *m V) *m (S *m V)^t* -
      (S *m V) *m diag_of (fun i : 'I_n => (lam i)^-1) *m (S *m V)^t*
    by rewrite mulmxBr mulmxBl mulmx1.
  rewrite /X -factor.
  suff -> : (1%:M : 'M[C]_n) - diag_of (fun i : 'I_n => (lam i)^-1) =
