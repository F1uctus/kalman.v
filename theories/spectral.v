...
Section Spectral.
  ...
  *)
  ...
  (*
    ...
    psd M -> v† M v = 0 -> M v = 0.
    ```
    Через спектральную теорему `M = U diag(l) U†` с `l_i >= 0`:
    `v† M v = Σ_i l_i |(U† v)_i|²`; зануление суммы неотрицательных слагаемых
    даёт `l_i (U† v)_i = 0` для всех i, откуда `diag(l) (U† v) = 0` и
    `M v = U (diag(l) (U† v)) = 0`. Это факт о том, что для неотрицательно
    определённой матрицы зануление формы ⟺ зануление столбца, нужный для
    перехода от энергетического баланса Ляпунова к PBH-условию.
  *)
  ...
  (*
    Для любой неотрицательно определённой матрицы `A` выполнено `A <= (tr A) E`
    в порядке Лёвнера.
  *)
