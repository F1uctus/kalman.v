(*
  ...
  Spectral theorem for hermitian matrices over `numClosedFieldType`.

  Uses `mathcomp.algebra.spectral.orthomx_spectralP`: every hermitian matrix is
  unitary diagonalizable and all it's eigenvalues are real
...
Section Spectral.
  ...
  Definition diag n (l : 'I_n -> ℂ) : 'M[ℂ]_n :=
    \matrix_(i, j) (if i == j then l i else 0).

  Lemma diag_hermsym n (l : 'I_n -> ℂ) :
    (forall i, l i \is Num.real) -> diag l \is hermsymmx.
  Proof.
  ...
  Lemma diag1 n :
    diag (fun _ : 'I_n => (1 : ℂ)) = 1%:M.
  Proof.
    apply/matrixP=> i j; rewrite !mxE.
    by case: (i == j); rewrite ?mulr1n ?mulr0n.
  Qed.

  Lemma diagM n (f g : 'I_n -> ℂ) :
    diag f *m diag g = diag (fun i => f i * g i).
  Proof.
    apply/matrixP=> i j; rewrite !mxE.
  ...
  Lemma diagB n (f g : 'I_n -> ℂ) :
    diag f - diag g = diag (fun i => f i - g i).
  Proof.
    apply/matrixP=> i j; rewrite !mxE.
  ...
  (*
    ...
    Any hermitian matrix is diagonalizable with real eigenvalues: $exists U$ and
    real diagonal $D$, so that $M = U D U†$.
  ...
  *)
  Lemma spectral_decomp n (M : 'M[ℂ]_n) :
    ...
    exists (U : 'M[ℂ]_n) (l : 'I_n -> ℂ),
      ...
      M = U *m diag l *m U^t*.
  (* Reuses $"orthomx_spectralP"$ from $"mathcomp.algebra.spectral"$. *)
  Proof.
    move=> Mherm.
  ...
  *)
  ...
  Lemma diag_qform n (l : 'I_n -> ℂ) (v : 'cV[ℂ]_n) :
    \tr (v^t* *m diag l *m v) =
  ...
  Lemma psd_diagE n (l : 'I_n -> ℂ) :
    (forall i, l i \is Num.real) ->
    psd (diag l) <-> (forall i, 0 <= l i).
  Proof.
    move=> hreal; split.
    - case=> _ hQ i.
      have hi := hQ (delta_mx i ord0).
      rewrite diag_qform (bigD1 i) //= big1 ?addr0 in hi; last first.
        move=> j /negbTE neji.
      ...
      exact: hi.
    - move=> hl; split; first by apply: hermsym_eq; exact: diag_hermsym.
      move=> v; rewrite diag_qform.
      apply: sumr_ge0=> i _.
      apply: mulr_ge0 (hl i) _.
      by rewrite -normCKC exprn_ge0.
  Qed.

  Lemma psd_eigvals_ge0 n (M : 'M[ℂ]_n) :
    M \is hermsymmx ->
    (psd M <->
    exists (U : 'M[ℂ]_n) (l : 'I_n -> ℂ),
      U \is unitarymx /\ (forall i, l i \is Num.real) /\
      M = U *m diag l *m U^t* /\ (forall i, 0 <= l i)).
  Proof.
    move=> hSym.
    have [U [l [hU [hreal hEq]]]] := spectral_decomp hSym.
    split.
    - move=> hPsd.
      have hPsdD : psd (diag l).
        have hEqRev : U^t* *m M *m U = diag l.
          rewrite hEq.
          have e1 : U^t* *m (U *m diag l *m U^t*) =
                    (U^t* *m U) *m diag l *m U^t*
            by rewrite !mulmxA.
          rewrite e1.
          have h := unitary_mulV hU.
          by rewrite !h !mul1mx -mulmxA h mulmx1.
        have -> : diag l = U^t* *m M *m U by rewrite hEqRev.
        exact: psd_congr hPsd.
      exists U, l; do !split=> //.
      by apply/(psd_diagE hreal).
  ...
  (*
    Обращение в ноль квадратичной формы неотрицательно определённой матрицы
    влечёт обращение в ноль столбца: $"psd" M -> v† M v = 0 ==> M v = 0$.
  *)
  Lemma psd_qf0_mul0 n (M : 'M[ℂ]_n) (v : 'cV[ℂ]_n) :
    psd M -> \tr (v^t* *m M *m v) = 0 -> M *m v = 0.
  (*
    Через спектральную теорему
    $M = U "diag"(l) U†, l_i >= 0 : v† M v = Σ_i l_i |(U† v)_i|²$; обращение в
    ноль суммы неотрицательных слагаемых даёт $forall i: l_i (U† v)_i = 0$,
    откуда $"diag"(l) (U† v) = 0$ и $M v = U ("diag"(l) (U† v)) = 0$. Это факт о
    том, что для неотрицательно определённой матрицы обращение в ноль формы ⟺
    обращение в ноль столбца, нужный для перехода от энергетического баланса
    Ляпунова к критерию PBH.
  *)
  Proof.
    move=> Mpsd qf0.
    ...
    have lnn : forall i, 0 <= l i.
      apply/(psd_diagE lreal).
      have e : diag l = U^t* *m M *m U.
        rewrite Meq.
        have e1 : U^t* *m (U *m diag l *m U^t*) *m U
                = (U^t* *m U) *m diag l *m (U^t* *m U) by rewrite !mulmxA.
        by rewrite e1 hUV !mul1mx mulmx1.
      rewrite e; exact: psd_congr Mpsd.
    set u := U^t* *m v.
    have qf_eq : \tr (v^t* *m M *m v) = \tr (u^t* *m diag l *m u).
      by rewrite Meq /u trmxC_mul trmxCK !mulmxA.
    rewrite qf_eq diag_qform in qf0.
  ...
  Lemma spec_conjM n (U D1 D2 : 'M[ℂ]_n) :
    U \is unitarymx ->
  ...
  Lemma invmx_spec_conj n (U : 'M[ℂ]_n) (l : 'I_n -> ℂ) :
    ...
    invmx (U *m diag l *m U^t*) =
      U *m diag (fun i => (l i)^-1) *m U^t*.
  Proof.
    move=> hU hl.
    pose L := U *m diag l *m U^t*.
    pose M := U *m diag (fun i => (l i)^-1) *m U^t*.
    have prod_id : L *m M = 1%:M.
      rewrite /L /M spec_conjM // diagM.
      suff -> : diag (fun i : 'I_n => l i * (l i)^-1) =
                (1%:M : 'M[ℂ]_n).
        by rewrite spec_conj1.
      rewrite -[RHS]diag1.
      apply: eq_diag=> i /=.
  ...
  Lemma spec_conjK n (A U : 'M[ℂ]_n) (l : 'I_n -> ℂ) :
    U \is unitarymx -> A = U *m diag l *m U^t* ->
    U^t* *m A *m U = diag l.
  Proof.
    move=> hU hAdec.
    rewrite hAdec.
    have e2 : U^t* *m (U *m diag l *m U^t*) *m U =
              U^t* *m U *m diag l *m U^t* *m U
      by rewrite !mulmxA.
    have h := unitary_mulV hU.
    by rewrite e2 !h !mul1mx -mulmxA h mulmx1.
  Qed.

  Lemma pd_eigvals_gt0 n (A U : 'M[ℂ]_n) (l : 'I_n -> ℂ) :
  ...
  Proof.
    ...
    have UAU : U^t* *m A *m U = diag l := spec_conjK hU hAdec.
    have hVAV : v^t* *m A *m v = ei^t* *m diag l *m ei.
      by rewrite /v -UAU trmxC_mul -!mulmxA.
    have lqf : \tr (ei^t* *m diag l *m ei) = l i.
      rewrite diag_qform (bigD1 i) //= big1 ?addr0; last first.
        move=> j /negbTE neji.
        by rewrite !mxE neji /= conjC0 mul0r mulr0.
  ...
  Lemma psd_spec_conjK n (V X : 'M[ℂ]_n) :
  ...
  *)
  Proof.
    ...
    have lpos : forall i, 0 < l i := pd_eigvals_gt0 pdA hU hreal hAdec.
    ...
    pose S : 'M[ℂ]_n := U *m diag isl *m U^t*.
    have isl_real : forall i, isl i \is Num.real
      by move=> i; rewrite /isl realV.
    have Sherm : S \is hermsymmx.
      rewrite /S.
      have hD : diag isl \is hermsymmx := diag_hermsym isl_real.
      have hH : (U *m diag isl *m U^t*) \is hermsymmx.
        have := @hermsym_congr ℂ n (U^t*) (diag isl) hD.
        by rewrite trmxCK.
      exact: hH.
    ...
    have SAS_eq : S *m A *m S = 1%:M.
      rewrite /S hAdec.
      have step1 : (U *m diag isl *m U^t*) *m (U *m diag l *m U^t*) =
                  U *m (diag isl *m diag l) *m U^t*
        := spec_conjM _ _ hU.
      rewrite step1.
      have step2 : (U *m (diag isl *m diag l) *m U^t*) *m
                  (U *m diag isl *m U^t*) =
                  U *m ((diag isl *m diag l) *m diag isl) *m U^t*
        := spec_conjM _ _ hU.
      rewrite step2 !diagM.
      suff -> : diag (fun i : 'I_n => isl i * l i * isl i) =
                (1%:M : 'M[ℂ]_n).
        exact: spec_conj1.
      by rewrite -[RHS]diag1; apply: eq_diag.
    have S_unit : S \in unitmx by have [_ h] := mulmx1_unit SAS_eq.
    have invA_eq : invmx A = S *m S.
      rewrite hAdec invmx_spec_conj //.
      rewrite /S spec_conjM // diagM.
      congr (U *m _ *m U^t*).
      apply: eq_diag=> i /=.
      rewrite /isl -invfM.
      have -> : sqrtC (l i) * sqrtC (l i) = l i by rewrite -expr2 sqrtCK.
    ...
    have lam_ge1 : forall i, 1 <= lam i.
      ...
      have key : B' - 1%:M = V *m diag (fun j : 'I_n => lam j - 1) *m V^t*.
        rewrite B'dec.
        rewrite -[X in _ - X](spec_conj1 hV).
        rewrite -mulmxBl -mulmxBr.
        suff -> : (diag lam : 'M[ℂ]_n) - 1%:M =
                  diag (fun j : 'I_n => lam j - 1) by [].
        by rewrite -diag1 diagB.
      rewrite key in hpsd0.
      have psdD : psd (diag (fun j : 'I_n => lam j - 1)).
        have := psd_spec_conjK hV hpsd0; by [].
      ...
      by move=> i; rewrite -subr_ge0;
        exact: (proj1 (psd_diagE lamreal_sub) psdD i).
    ...
    pose X : 'M[ℂ]_n := (S *m V) *m
                        diag (fun i : 'I_n => (lam i)^-1) *m (S *m V)^t*.
    have X_alt : X = S *m V *m diag (fun i : 'I_n => (lam i)^-1) *m V^t* *m S^t*.
      by rewrite /X trmxC_mul !mulmxA.
    have hSBS_eq : S *m B *m S = V *m diag lam *m V^t* by rewrite -/B'.
    have SBX : S *m (B *m X) = S^t*.
      rewrite X_alt.
      have eq1 :
        S *m (B *m (S *m V *m diag (fun i : 'I_n => (lam i)^-1) *m V^t* *m S^t*)) =
        S *m B *m S *m V *m diag (fun i : 'I_n => (lam i)^-1) *m V^t* *m S^t*
        by rewrite !mulmxA.
      rewrite eq1 hSBS_eq.
      have eq2 :
        V *m diag lam *m V^t* *m V *m diag (fun i : 'I_n => (lam i)^-1)
          *m V^t* *m S^t* =
        V *m diag lam *m (V^t* *m V) *m diag (fun i : 'I_n => (lam i)^-1)
          *m V^t* *m S^t*
        by rewrite !mulmxA.
      rewrite eq2 (unitary_mulV hV) mulmx1.
      rewrite -[V *m diag lam *m diag _]mulmxA diagM.
      have -> : diag (fun i : 'I_n => lam i * (lam i)^-1) =
                (1%:M : 'M[ℂ]_n).
        rewrite -[RHS]diag1; apply: eq_diag=> i /=.
    ...
    have diff_eq : invmx A - X =
        (S *m V) *m diag (fun i : 'I_n => 1 - (lam i)^-1) *m (S *m V)^t*.
      rewrite invA_eq -SVSV_eq.
      have factor :
        (S *m V) *m ((1%:M : 'M[ℂ]_n) - diag (fun i : 'I_n => (lam i)^-1))
          *m (S *m V)^t* =
        (S *m V) *m (S *m V)^t* -
          (S *m V) *m diag (fun i : 'I_n => (lam i)^-1) *m (S *m V)^t*
        by rewrite mulmxBr mulmxBl mulmx1.
      rewrite /X -factor.
      suff -> : (1%:M : 'M[ℂ]_n) - diag (fun i : 'I_n => (lam i)^-1) =
                diag (fun i : 'I_n => 1 - (lam i)^-1) by [].
      by rewrite -diag1 diagB.
    rewrite /psd_le -X_eq_invB diff_eq.
    apply: psd_lcongr.
    have sub_real : forall i, 1 - (lam i)^-1 \is Num.real
      by move=> i; rewrite rpredB // realE; apply/orP; left; exact: ler01.
    apply/(psd_diagE sub_real)=> i.
    rewrite subr_ge0.
    have : (lam i)^-1 <= 1.
  ...
  *)
  ...
  (*
    ...
    спектральной теореме (`spectral_decomp`) $M = U D U†$, где $U in U(n)$, а
  ...
  *)
  Proof.
    ...
    have UMU : U^t* *m (B - A) *m U = diag l := spec_conjK hU Mdec.
    have psd_diag_l : psd (diag l).
      rewrite -UMU.
      exact: psd_congr h1.
    have l_ge0 : forall i, 0 <= l i
      := proj1 (psd_diagE lreal) psd_diag_l.
    have UnMU : U^t* *m (A - B) *m U = - diag l.
  ...
  *)
  Proof.
    ...
    have psdD : psd (diag l).
      apply: (psd_spec_conjK hU); by rewrite -Adec.
    have lnn := proj1 (psd_diagE lreal) psdD.
    have diag_const : forall a : ℂ, diag (fun _ : 'I_n => a) = a *: 1%:M.
    ...
    have idU : (\tr A *: 1%:M : 'M[ℂ]_n)
            = U *m diag (fun _ => \tr A) *m U^t*.
      by rewrite diag_const -scalemxAr -scalemxAl (spec_conj1 hU).
    have decomp : \tr A *: 1%:M - A
                = U *m diag (fun i => \tr A - l i) *m U^t*.
      rewrite (_ : \tr A *: 1%:M - A
                = U *m diag (fun _ => \tr A) *m U^t*
                  - U *m diag l *m U^t*); last first.
        by congr (_ - _); [exact: idU | exact: Adec].
      by rewrite -mulmxBl -mulmxBr diagB.
