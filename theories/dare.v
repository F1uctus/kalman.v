(*
  ...
  1.  Равномерная верхняя оценка `P_iter_bound` - для каждой итерации
      ...
      `riccati_step Σ <= Mc Σ Mc† + Wc`, откуда
      `iter k riccati_step 0 <= lyap_partial Mc Wc k <= Pbnd`, где `Pbnd` -
      равномерная мажоранта из суммируемости грамиана при устойчивости по Шуру
      (`lyap_partial_le_bnd_schur`, spec_rad.v; [kailath2000], § 14.5).
  2.  `riccati_iter_mono_from_0` даёт монотонность.
  3.  `mx_mono_cvgn` - сходимость в матричной топологии.
  4.  `cvgn_riccati_step` (этого файла, встроенное доказательство) - непрерывность
      шага Риккати на неотрицательно определённых входах. Композиция с
      `Pss_cvgn` и единственностью предела (`cvg_unique` + `norm_hausdorff`)
      даёт неподвижную точку.
  5.  Положительная определённость `Pss` (`Pss_pd`) выводится из полной
      ...
      (`riccati_closed_loop_identity`) положительно определена через
      грамиан управляемости замкнутого контура
  ...
  `cvgn_riccati_step` относится к другой константе. Чтобы переиспользовать тот же
  механизм для `riccati_step` из `kalman.v` без ручного приведения `change`, мы
  собираем непрерывность напрямую из элементарных `cvgn_addmx` / `cvgn_mulmx` /
...
Section DARE.
  ...
  *)
  ...
  (*
    ...
    установившейся ковариации `Pss`.

    ([kailath2000], App. E, Theorem E.5.1) полная управляемость пары процессного
  ...
  *)
  ...
  (*
    Обнаруживаемость влечёт стабилизирующее усиление и устойчивость по Шуру
    замкнутого контура ([kailath2000], § 14.5).

    Требуем обнаруживаемость пары F_detectable : detectable F H
    ...
    K0†, откуда следует равномерная мажоранта `Pbnd` (через суммируемость
    грамиана при устойчивости по Шуру, `lyap_partial_le_bnd_schur`). Суперрешение
    (матрица `X` с `riccati_step X <= X`, мажорирующая итерацию сверху) строится
    как `lyap_sol Mc Wc + a (lyap_sol Mc E)` (см. ниже): это возмущение по
    Ляпунову замкнутого контура, корректное для любого устойчивого по Шуру `Mc`.
  *)
  Hypothesis F_detectable : detectable F H.

  (*
    Стабилизирующий коэффициент усиления `K0` извлекается из обнаруживаемости
  ...
  *)
  ...
  (*
    ...
    ([kailath2000], § 14.2; App. E, Lemma E.3.1)
  *)
  ...
  *)
  ...
  (*
    ...
    ([kailath2000], § 14.2, факт (b); § 14.1.3; Prob. 14.4)
    ============================================================================
    ...
    Наша `iter k riccati_step 0` - это в точности `P°_i` (14.2.1), а
    `Pss = mx_mono_lim` - её предел `P°`. Эта же оценка снизу `P°_i`
  ...
  *)
  Section FixedGainBound.
    ...
    *)
    ...
    (*
      ...
      `update_cov <= alt_update_cov K`
      (разность - конгруэнция неотрицательно определённых матриц `(K − K_калман) Re (·)†`),
      далее монотонность `predict_cov` и тождество
      `F alt_update_cov K (Σ F† + G Q G†) = M Σ M† + W`.
    *)
    ...
    *)
    ...
    (*
      Установившаяся форма `Π = lyap_sol M W` при устойчивости по Шуру
    ...
    *)
    ...
    (*
      Оптимальность Калмана ([kailath2000], § 14.2, факт (b)): установившаяся
      ковариация `Pss = P°` не превосходит ss-ковариацию ошибки любого
      стабилизирующего наблюдателя `Π = lyap_sol M W` - «наблюдатель с усилением
      K не может превзойти оптимальный фильтр Калмана». Предел поэлементной
      оценки `P°_i <= Π` (мажоранта Π постоянна по i) через `mx_mono_lim_le`.
    *)
  ...
  (*
    ...
    ============================================================================
    Используем элементарные continuity-леммы из mxtopo и riccati_cont
  ...
  *)
  ...
  (*
    ...
    `(fun k => riccati_step (Pseq k)) @ ∞ --> riccati_step Pss`
    (по непрерывности) и одновременно `(fun k => Pseq k.+1) @ ∞ --> Pss`
    (по `cvg_shiftS` из `Pss_cvgn`). Эти две функции равны, значит пределы равны
  ...
  *)
  ...
  (*
    ...
    Это стандартный результат классической теории ДАУР ([kailath2000], § 14.5),
    требующий устойчивости замкнутого контура; у нас он доказан (теорема ниже)
    через суперрешение + теорема о двух милиционерах при спектральной
    устойчивости по Шуру `spec_rad_lt1 Mc`.
    ...
    2.  Единственность положительно определённой неподвижной точки: любая
        ...
        одновременно к `Pss`, откуда `Pi = Pss`
  ...
  *)
  ...
  (*
    ...
    имеем `L = Pss`. Для произвольного неотрицательно определённого `P0` берём a
    := tr P0 (тогда `P0 <= a E <= a Yinf <= Xinf + a Yinf`, т.к. `E <= Yinf`);
    теорема о двух милиционерах с `iter k r.s. 0` и оценкой `frob_sq <= (tr)²`
    даёт сходимость к `Pss`. Суперрешение работает при спектральной устойчивости
    по Шуру замкнутого контура `spec_rad_lt1 Mc`
    (а не системной `frob_sq F < 1`): возмущение по Ляпунову `Yinf` заменяет
    скаляр `E`, не требуя оценки операторной нормы `Mc`.

    Единственность положительно определённой неподвижной точки. Классический
    результат теории ДАУР ([kailath2000], § 14.5): любая положительно
    определённая неподвижная точка совпадает с Pss. Доказана через Lemma 14.4.1
    (Local Identities) для разности двух решений `riccati_step_fix_unique`
    (riccati_unique.v): разность предсказанных ковариаций удовлетворяет
    `M2 − M2 = Fp(M2)(M2−M2)Fp(M2)†` с обоими устойчивыми по Шуру контурами
  ...
  *)
  ...
  (*
    ============================================================================
    Суперрешение Риккати: `lyap_sol Mc Wc + a (lyap_sol Mc E) (a >= 0)`.
    ============================================================================
    `Xinf := lyap_sol Mc Wc` - решение уравнения Ляпунова замкнутого контура
    `Xinf = Mc Xinf Mc† + Wc`; `Yinf := lyap_sol Mc E` - положительно
    определённое возмущение, `Yinf = Mc Yinf Mc† + E >= E`. Тогда для любого
    вещественного `a >= 0` матрица `Xinf + a Yinf` - суперрешение:
    ```
    riccati_step (Xinf + a Yinf) <= Mc (Xinf + a Yinf) Mc† + Wc
        = Xinf + a (Mc Yinf Mc†) = Xinf + a (Yinf − E)
        = (Xinf + a Yinf) − a E <= Xinf + a Yinf.
    ```
  ...
  *)
  ...
  (*
    ============================================================================
    Верхняя траектория из `Xinf + a E` - монотонно убывает к Pss
  ...
  *)
  ...
  (*
    Сходимость усиления Калмана для произвольного неотрицательно определённого
    начала.
  *)
  ...
  (*
    Полная топологическая теорема ДАУР: существование, положительная
    определённость, ограниченность, глобальная сходимость и единственность
    стабилизирующего решения.

    ([kailath2000], App. E, Theorem E.5.1)
  *)
  ...
  (*
    ============================================================================
    Связь матричной топологии с нормой Фробениуса и теорема ДАУР
    ============================================================================
    Топологическая сходимость `f @ ∞ --> L` в матричной топологии эквивалентна
    стремлению квадрата нормы Фробениуса `tr((f k - L)† ⋅ (f k - L))` к нулю.
    Этот факт даёт одну сторону эквивалентности: из сходимости в матричной
    топологии следует стремление квадрата нормы Фробениуса к нулю.
  *)
  ...
  (*
    Схема:
    1.  `(Pf k - L) @ ∞ --> 0` через `cvgn_submx` + `cvg_cst`.
    2.  `(Pf k - L)† @ ∞ --> 0† = 0` через `cvgn_trmxC`.
    3.  `(Pf k - L)† ⋅ (Pf k - L) @ ∞ --> 0` через `cvgn_mulmx`.
    4.  `tr(...) @ ∞ --> tr 0 = 0` через `cvgn_mxtrace`.
    5.  Переход к скалярному типу `ℂ^o`, применение `cvgrPdistC_lt` даёт
        `\near ∞, |tr(...) - 0| < eps`.
    6.  `tr(...) >= 0` (frob_sq_ge0 из mxfrob/kalman) + `ger0_norm` убирают
        абсолютную величину; «near ∞» на nat-фильтре раскрывается в
        `exists N, forall k >= N, ...`.
  *)
  Proof.
  ...
  (*
    Теорема ДАУР: сходимость по норме Фробениуса в форме определения предела
    (для любого ε > 0 найдётся N такое, что отклонение мало при k >= N).

    ([kailath2000], App. E, Theorem E.5.1; § 14.5, Theorem 14.5.1)
  *)
  ...
  (*
    ...
    Откуда непосредственно следует сжатие в порядке Лёвнера: psd_le
    (Fp ⋅ P_pss ⋅ Fp†) P_pss.

    ============================================================================
    Сокращения `P_pss`, `Kf`, `Kp`, `Fp`, `R_e` и базовые тождества замкнутого
    контура: `P_pss_psd`, `Kf_R_e_eq`, `Pss_eq_update`,
    `predict_cov_closed_loop`, `Fp_Ppss_Ht`, `riccati_closed_loop_identity` выше
    (у `Pss_pd`): `Pss_pd` выводится из управляемости через
  ...
  *)
  ...
  (*
    ...
    (предикторная форма `F - Kp H`). Энергия этого распространения в норме с
    весом `R⁻¹` равна бесконечной сумме
    `∑_k (Fp†)^k (H† R⁻¹ H) Fp^k`, т.е. бесконечным грамианом наблюдаемости
    замкнутого контура под весом `H† R⁻¹ H`. Сходимость суммы обеспечена
    устойчивостью по Шуру `Fp_schur`, через устойчивый по Шуру
    `lyap_sol_*_schur`; невырожденность веса, а с ней положительная
    определённость `O_P` - наблюдаемостью пары `[Fp, H]`.
  *)
  ...
  *)
  ...
  (*
    ...
    сходится к `Pss`, а матрица усиления Калмана - к стационарной, по
    норме Фробениуса (следствие `riccati_steady_state_proven`). Один шаг
    фильтра двигает ковариацию ровно как `riccati_step` (по определению:
    ```
