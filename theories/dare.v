(*
  ...
  предсказанная ss-ковариация `P_pss = Fp P_pss Fp† + (Kp R Kp† + GQG†)`
  ...
  затем `Pss = update_cov P_pss` сохраняет положительную определённость
  (`update_cov_pd`).
*)
...
Section DARE.
  ...
  *)
  ...
  (*
    ...
    определённость `Pss`. Существование лишь _неотрицательно_ определённого
  ...
  *)
  ...
  (*
    ...
    устойчивым (`K_p := F K`), нулевая Риккати-итерация мажорируется сверху P°_i
    <= Π, Π = M Π M† + (G Q G† + K_p R K_p†), где `Π = lyap_sol M W` -
  ...
  *)
  Section FixedGainBound.
    ...
    *)
    ...
    (*
      Оптимальность Калмана: установившаяся ковариация `Pss = P°` не превосходит
      ss-ковариацию ошибки любого стабилизирующего наблюдателя
      `Π = lyap_sol M W` - «наблюдатель с усилением K не может превзойти
      оптимальный фильтр Калмана». Предел поэлементной оценки `P°_i <= Π`
      (мажоранта Π постоянна по i) через `mx_mono_lim_le`.
  ...
  *)
  ...
  (*
    Положительная определённость `Pss`: полная управляемость пары `(F, G Q^½)`
    => `Pss > 0`.
  ...
  *)
  ...
  (*
    ...
    предсказанных ковариаций удовлетворяет `M1 − M2 = Fp(M1)(M1−M2)Fp(M2)†` с
  ...
  *)
  ...
  (*
    ...
    ```
    - ([kailath2000], § 14).
    Для произвольного начала `P0` отклонение ковариации
