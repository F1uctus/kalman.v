(*
  ...
  `numDomainType`. Математическое ожидание случайной матрицы задаётся формулой
  оператора `Ex` из `probability.proba` (сумма значений с весами `μ ω`);
  свойства линейности доказываются дословно как леммы `E_add_RV`, `E_scale_RV`,
  `E_opp_RV` из infotheo. На вещественном поле наш оператор совпадает с `E из
  infotheo с точностью до конвертируемости, а свойства линейности выводятся из
  одноимённых лемм библиотеки (секция ExpectationInfotheo ниже).

  Оператор `Ex` из infotheo напрямую не применим: он объявлен для
  `R : realType`, тогда как поле скаляров фильтра Калмана есть алгебраически
  замкнутое `numClosedFieldType`, которое вещественным полем не является.
  Поэтому ожидание определено здесь заново той же формулой, но над
  `numDomainType`, что покрывает оба случая.
*)
...
Section Expectation.
  ...
  *)
  ...
  (*
    ...
    Единственное место, где используется полная масса распределения (`FDist.f1`:
    $sum_ω μ(ω) = 1$); ср. `E_const_RV` в infotheo.
  *)
...
(*
  Распределение $μ$ входит только в заключение лемм, поэтому неявным аргументом
...
Section ExpectationInfotheo.
  ...
  (*
    Мост к infotheo: на вещественном поле наш оператор совпадает с `E, а
    свойства линейности следуют из одноимённых лемм библиотеки.
  *)
