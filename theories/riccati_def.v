(*
  ...
  `numClosedFieldType`, а вычислимое уточнение (`seqmx/riccati.v`) подставляет
  `conj := idfun` над полем `rat` или арифметикой `bigQ`.
  ...
  имя оператора закрыто обозначением, фиксирующим `conj := conjC` (тогда
  `rewrite /predict_cov` не сработает, т.к. `/` снимает определение, но не
  обозначение).
*)
