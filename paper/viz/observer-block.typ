...
#let observer-block-canvas(st, theme: (:)) = cetz.canvas(length: 1.05cm, {
  import cetz.draw: circle, content, line, rect

  let wire-paint = theme.at("wire", default: black)
  let box-paint = theme.at("box", default: gray)
  let mark-fill = theme.at("mark-fill", default: black)
  let tap-fill = theme.at("tap-fill", default: black)
  let label-fill = theme.at("label-fill", default: auto)
  let region-fill = theme.at("region-fill", default: white)
  let region-caption-fill = theme.at("region-caption-fill", default: gray.darken(25%))
  let show-region-labels = theme.at("show-region-labels", default: true)

  let wire = (paint: wire-paint, thickness: 0.8pt)
  let boxst = (paint: box-paint, dash: "dashed", thickness: 0.8pt)
  let mk = (end: ">", scale: 0.55, fill: mark-fill)
  let rsum = 0.16
  let lbl(body) = if label-fill == auto {
    text(size: st.label, body)
  } else {
    text(size: st.label, fill: label-fill, body)
  }

  // Прямоугольный блок с подписью.
  let blk(cx, cy, w, h, label) = {
    rect((cx - w / 2, cy - h / 2), (cx + w / 2, cy + h / 2), stroke: wire)
    content((cx, cy), lbl(label))
  }
  ...
  if show-region-labels {
    content(
      ...
      box(
        fill: region-fill,
        inset: (x: 2.5pt),
        text(size: st.label + 1pt, fill: region-caption-fill)[Система],
      ),
    )
  }

  // вход управления и точка ветвления к наблюдателю
  content((-6.55, 2.2), lbl($bold(u)_k$))
  line((-6.3, 2.2), (-4.96, 2.2), stroke: wire, mark: mk)
  tap(-5.7, 2.2)

  // шум процесса
  content((-4.8, 3.2), lbl($bold(w)_(k+1)$))
  ...
  tap(0.1, 2.2)
  content((0.1, 2.48), lbl($bold(x)_k$))
  ...
  if show-region-labels {
    content(
      ...
      box(
        fill: region-fill,
        inset: (x: 2.5pt),
        text(size: st.label + 1pt, fill: region-caption-fill)[Наблюдатель],
      ),
    )
  }
...
#let observer-block-figure(style: (:), theme: (:)) = {
  let st = viz-resolve(style)
  let canvas = observer-block-canvas(st, theme: theme)
  layout(size => {
