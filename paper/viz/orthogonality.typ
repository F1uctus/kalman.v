...
// perpendicular to Y (the normal equation kalman.kalman_gain_normal_eq,
...
// the square-completion identity behind kalman.kalman_gain_optimal:
...
#let orthogonality-figure(data: orthogonality-data, style: (:)) = {
  ...
  content(
    ...
    text(size: st.annot, fill: rgb(
      ...
      55,
      30,
    ))[$"Tr" P_(s|s-1) = #calc.round(tr-alt, digits: 2)$],
    anchor: "south-east",
    padding: 0.14,
  )
  ...
  content(
    ...
    text(size: st.annot)[$"Tr" P_(s|s) = #calc.round(tr-opt, digits: 2)$],
    anchor: "west",
    padding: 0.14,
  )
