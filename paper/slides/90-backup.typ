...
- Обновление ковариации допускает запись через обратные матрицы
  (информационная форма); из неё немедленно следует обратимость
  апостериорной ковариации
...
#two-col(
  columns: (1fr, 300pt),
  [
    - Спектральная теорема для эрмитовых
      матриц (`spectral_decomp`)
    - Обращение антимонотонно относительно
      порядка Лёвнера (`pd_inv_antimono`):
      из $A prec.eq B$ следует
      $B^(-1) prec.eq A^(-1)$
    - Используется в доказательствах
      единственности решения ДАУР
  ],
...
Спектры замкнутых контуров наблюдателя и регулятора совпадают:
двойственность переносит устойчивость между задачами оценивания
и управления.
...
- Скалярное ДАУР из $P_0 = 1$ даёт последовательность
  $1 -> 5\/6 -> 13\/16$
- Значение вычислено `vm_compute` внутри #Rocq и связано
  со спецификацией уточнением `ex_iter_correct`
...
- Стационарное $P_(s s)$ невычислимо как предел: берётся конечная
  итерация `riccati_step` до малой невязки неподвижной точки
- Прогон фильтра стохастичен: коридор $plus.minus 2 sigma_k$ есть
  точная извлечённая ковариация, а траектория и измерения получены
  посевным генератором псевдослучайных чисел (численная симуляция)
- Параметры модели $F, G, H, Q, R, P_0$ остаются аргументами
  извлечённых функций: настройка на этапе вызова
- Подход через CoqEAL не зависит от внешнего экстрактора и даёт
  формализованное соответствие спецификации и кода
...
#table(
  ...
  [Оптимальность усиления], [`kalman_gain_optimal`], [`kalman.v`],
  [Грамиан и наблюдаемость], [`obsv_gram_pd_of_observable`],
  [`obsv_bound.v`],
  ...
  [Уточнения CoqEAL], [`riccati_iter_seqmx_correct`],
  [`riccati_seqmx.v`],
