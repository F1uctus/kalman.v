...
#two-col(
  columns: (1fr, 270pt),
  [
    - Ранговые критерии: пара $(H, F)$ наблюдаема, а пара $(F, G)$ управляема,
      если соответствующие блочные матрицы имеют полный ранг $n$
    - Грамианы дают конструктивную форму: суммы неотрицательно определённых
      слагаемых
    - При наблюдаемости грамиан из $n$ слагаемых положительно определён
      (`obsv_gram_pd_of_observable`), симметрично для управляемости
  ],
...
#two-col(
  ...
  [
    ...
    - Двойственность оценивания и управления: `stabilizable_dual`,
      `ctrl_gram_dual` переносят факты между парами $(H, F)$ и $(F^*, H^*)$
  ],
