...
#two-col(
  ...
  [
    ...
    Шумы с нулевым средним и известными
    ковариациями:
    $
    ...
    Критерий качества: ковариация ошибки
    $EE err(x)_(k|k) err(x)_(k|k)^*$ минимальна
    в порядке Лёвнера.
  ],
...
- Модель, шаги фильтра и их свойства оформлены как определения
  и теоремы в `.v` файлах поверх MathComp
- Доказательства проверяются ядром #Rocq. Текст ВКР цитирует
  утверждения напрямую из исходников
