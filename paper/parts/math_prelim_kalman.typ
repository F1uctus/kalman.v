...
Ниже зафиксированы определения и их соответствия в системе Rocq. Доказательства в тексте опускаются; они приведены в формализации. Единичную матрицу порядка $n$ обозначаем
$E_n$ (как в ряде учебников по линейной алгебре); в Rocq она записывается как
`1%:M` в контексте квадратных матриц #raw("'M[R]_n").

== X.1. Вещественные скаляры и матрицы
...
(размер выводится из контекста), что ниже сопоставляется с $E_n$.
...
== X.5. Информационная форма (обратие апостериорной ковариации)
